...
def load_risk():
    return pd.read_csv(
        '../data/processed/risk_metrics.csv', index_col=0)
...
def load_predictions():
    return pd.read_csv(
        '../data/processed/hdfcbank_predictions.csv')
...
if page == "🏠 Home":
    ...
    # Calculate correct best performer
    stock_returns = {}
    ...
    fig.add_vline(x=2008, line_dash='dash',
                  line_color='red', annotation_text='2008 Crisis')
    fig.add_vline(x=2020, line_dash='dash',
                  line_color='orange', annotation_text='COVID-19')
    fig.update_layout(
        xaxis_title='Year',
    ...
    msg = f"Despite crashes, NIFTY-50 grew {total_growth:.0f}% overall from 2000 to 2021 — proving long term investing works"
    col3.info(f"**💰 Long Term Growth**\n\n{msg}")
...
# ══════════════════════════════════════════════════════
    ...
    stock_df = stock_df[
        stock_df['Date'].dt.year >= start_year
    ].sort_values('Date')
    ...
    if len(stock_df) == 0:
        ...
        col3.metric("📉 Volatility",
                    f"{latest['Volatility_20']*100:.1f}%")
        col4.metric("⚡ Momentum",
                    f"{latest['Momentum_20']*100:.1f}%")
        col5.metric("📦 Volume Ratio",
        ...
        if indicator == "Moving Averages":
            ...
            fig.add_trace(go.Scatter(
                ...
                name='MA 200', line=dict(
                    color='purple', width=1.5, dash='dot')
            ))
...
# ══════════════════════════════════════════════════════
    ...
    st.markdown(
        "Build optimized portfolios using Modern Portfolio Theory")
    st.markdown("---")
    ...
    # Profile description
    descriptions = {
        "Conservative": "🟢 Low risk, stable returns. Suitable for retired investors or those who cannot afford losses. Focus on stable blue-chip stocks.",
        "Balanced":     "🔵 Moderate risk and returns. Suitable for working professionals. Mix of growth and stability.",
        "Aggressive":   "🔴 High risk, high returns. Suitable for young investors with long time horizons. Focus on high-growth stocks."
    }
    ...
    weights      = weights[weights > 1].sort_values(
        ascending=False)
    ...
    available = [s for s in port_indexed.index
    ...
    for col, prof in zip([col1, col2, col3],
                         ['Conservative', 'Balanced', 'Aggressive']):
        m = metrics_data[prof]
        col.markdown(f"**{prof}**")
    ...
    for prof in ['Conservative', 'Balanced', 'Aggressive']:
        ...
        fig.add_trace(go.Scatter(
            x=ret_port.index,
            y=cum,
            name=f'{prof} → ₹{cum.iloc[-1]:,.0f}',
            line=dict(
                color=profile_colors[prof], width=2.5)
        ))
    ...
    fig.add_hline(
        ...
        line_dash='dot',
        line_color='gray',
        annotation_text=f'Initial: ₹{investment:,}'
    )
    fig.update_layout(
        title=f'Growth of ₹{investment:,} Investment',
        xaxis_title='Date',
...
# ══════════════════════════════════════════════════════
    ...
    st.markdown(
        "Comprehensive risk analysis for individual stocks and portfolios")
    st.markdown("---")

    tab1, tab2 = st.tabs(
        ["📊 Individual Stock Risk", "💼 Portfolio Risk"])
        ...
        with col1:
            ...
            if symbol in risk_df.index:
                ...
                st.metric("📈 Annual Return",
                          f"{r['Annual Return (%)']:.1f}%")
                st.metric("📊 Annual Volatility",
                          f"{vol:.1f}%",
                          "Low" if vol < 25 else
                          "High" if vol > 35 else "Medium")
                st.metric("⭐ Sharpe Ratio",
                          f"{sharpe:.2f}",
                          "Excellent" if sharpe > 1 else
                          "Good" if sharpe > 0.5 else "Poor")
                ...
                st.metric("📐 Beta",
                          f"{r['Beta']:.2f}",
                          "Aggressive" if r['Beta'] > 1.2
                          else "Defensive" if r['Beta'] < 0.8
                          else "Neutral")
                st.metric("🔄 Sortino Ratio",
                          f"{r['Sortino Ratio']:.2f}")
    ...
    with tab2:
        ...
        st.dataframe(
            risk_df[display_cols].sort_values(
                'Sharpe Ratio', ascending=False
            ).style.background_gradient(
                cmap='RdYlGn',
                subset=['Sharpe Ratio', 'Annual Return (%)']
            ).background_gradient(
                cmap='RdYlGn_r',
                subset=['Annual Volatility (%)',
                        'Max Drawdown (%)']
            ).format("{:.2f}"),
            use_container_width=True,
            height=500
...
# ══════════════════════════════════════════════════════
    ...
    st.markdown(
        "Deep Learning model trained on HDFCBANK stock data")
    st.markdown("---")
    ...
    try:
        ...
        col3.metric("🎯 R² Score",
                    f"{r2:.4f}",
                    "Higher is Better")
        col4.metric("🎲 Direction Accuracy",
                    f"{dir_acc:.1f}%",
                    "Higher is Better")
        ...
        fig.add_trace(go.Scatter(
            ...
            name='Actual Price',
            line=dict(color='#3498DB', width=2),
            hovertemplate='Day: %{x}<br>Actual: ₹%{y:.0f}<extra></extra>'
        ))
        fig.add_trace(go.Scatter(
            y=predicted,
            name='Predicted Price',
            line=dict(color='#E74C3C', width=2, dash='dash'),
            hovertemplate='Day: %{x}<br>Predicted: ₹%{y:.0f}<extra></extra>'
        ))
        fig.update_layout(
        ...
        fig.add_annotation(
            text=f'R²={r2:.3f} | RMSE=₹{rmse:.0f} | MAE=₹{mae:.0f}',
...
Train/Test: 80% / 20%
        ...
        with col2:
            ...
            # Use hardcoded baseline values
            # (baseline models trained separately)
            comp_data = pd.DataFrame({
                'Model': ['Linear Regression',
                          'Random Forest', 'LSTM (Ours)'],
            ...
            fig2 = px.bar(
                ...
                x='Model', y='R²',
                color='Type',
                title='R² Score — Model Comparison',
                color_discrete_map={
                    'Baseline':  '#95A5A6',
                    'Our Model': '#2ECC71'
                },
        ...
        st.error(
            ...
            "Please run notebook 03_stock_predictor.ipynb first."
        )
...
# ══════════════════════════════════════════════════════
    ...
    st.markdown(
        "Understanding WHY our model makes predictions")
    st.markdown("---")

    st.subheader("📊 Feature Importance Analysis")
    st.markdown(
        "Which technical indicators influence stock predictions most?")

    # Feature importance from correlation analysis
    symbol   = st.selectbox(
        "Select Stock", sorted(df['Symbol'].unique()))
    stock_df = df[df['Symbol'] == symbol].copy()
    ...
    st.markdown(
        "Our system generates BUY/SELL/HOLD signals "
    ...
    if score >= 2:
        ...
        color = "success"
        explanation = (
            f"Multiple indicators suggest {symbol} is in a "
            f"bullish phase. RSI, MACD and Moving Averages "
            f"are aligned positively."
        )
    elif score <= -2:
        ...
        explanation = (
            f"Multiple indicators suggest {symbol} is in a "
            f"bearish phase. Consider reducing exposure or "
            f"waiting for better entry point."
        )
    else:
        recommendation = "🟡 HOLD / WATCH"
        color = "warning"
        explanation = (
            f"Mixed signals for {symbol}. Indicators are not "
            f"aligned. Recommended to wait for clearer "
            f"confirmation before taking action."
        )
    ...
    st.caption(
        ...
        "Not financial advice. Always consult a certified "
    ...
    st.plotly_chart(fig, use_container_width=True)
    st.info(
        f"**{len(anomalies)} anomalous events** detected for "
        f"{symbol} where volatility exceeded 2 standard "
        f"deviations from the mean. These typically correspond "
        f"to major market events like the 2008 crisis and "
        f"2020 COVID crash."
    )